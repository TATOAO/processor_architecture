...
type EdgeData = { isAnimating?: boolean };
...
type ProcessorNodeData = {
  ...
  onAddConnectedNode?: (id: string) => void;
};
type ProcessorNodeType = Node<ProcessorNodeData, 'processor'>;

const ProcessorNode = ({ data, id }: NodeProps<ProcessorNodeType>) => {
  ...
  return (
    <>
      ...
      {contextMenu && (
        ...
        >
          <button onClick={handleEditName} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100">
            Edit Name
          </button>
          <button onClick={handleAddConnectedNode} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100">
            Add Connected Node
          </button>
          <button
            onClick={handleDelete}
      ...
      {showTypeSelector && <div className="fixed inset-0 z-40" onClick={() => setShowTypeSelector(false)} />}
    </>
  );
};
...
function GraphComponentInner({ graphData, onGraphChange }: GraphComponentProps) {
  ...
  const deleteNode = useCallback(
    (nodeId: string) => {
      ...
      setEdges((existingEdges) => existingEdges.filter((edge) => edge.source !== nodeId && edge.target !== nodeId));
      ...
      if (onGraphChange) {
        const newGraphData = {
          ...
          edges: graphData.edges.filter(
            (edge) => edge.source_node_unique_name !== nodeId && edge.target_node_unique_name !== nodeId
          ),
        };
  ...
  const addConnectedNode = useCallback(
    (sourceNodeId: string) => {
      ...
      setEdges((existingEdges) => [...existingEdges, newEdge]);
  ...
  const deleteSelected = useCallback(() => {
    setNodes((existingNodes) => existingNodes.filter((node) => !node.selected));
    setEdges((existingEdges) => existingEdges.filter((edge) => !edge.selected));
  }, [setNodes, setEdges]);
  ...
  return (
    <div className="w-full h-[600px] border border-gray-300 rounded-lg">
      <div className="p-2 bg-gray-100 text-xs">Debug: Nodes: {nodes.length}, Edges: {edges.length}</div>
      <ReactFlow
        nodes={nodes}
      ...
      >
        ...
        <Panel position="top-left" className="bg-white p-2 rounded shadow">
          <div className="flex gap-2">
            <button onClick={addNode} className="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-sm">
              Add Node
            </button>
            <button onClick={deleteSelected} className="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600 text-sm">
              Delete Selected
            </button>
            <button
              onClick={startAnimation}
              disabled={isAnimating}
              className={`px-3 py-1 text-white rounded text-sm ${isAnimating ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-500 hover:bg-green-600'}`}
            >
              Run Pipeline
            </button>
            <button
              ...
              disabled={!isAnimating}
              className={`px-3 py-1 text-white rounded text-sm ${!isAnimating ? 'bg-gray-400 cursor-not-allowed' : 'bg-orange-500 hover:bg-orange-600'}`}
            >
              Stop
            </button>
          </div>
